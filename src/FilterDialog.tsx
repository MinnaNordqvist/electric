...
export function FilterDialog({ isOpen, onClose, activeFilters, onApply, data }: Props) {
  ...
  // THE FIX: Rebuild activePairs from the activeFilters prop so the dialog 
  // is perfectly in sync even when chips are deleted in the DataTable!
  useEffect(() => {
    setDraft(activeFilters);

    const rebuiltPairs: ActiveFilter[] = [];
    Object.entries(activeFilters).forEach(([key, val]) => {
      if (val !== "" && val !== undefined) {
        let displayValue = val;
        // Map the month value back to the string label
        if (key === "month") {
          const m = MONTHS.find((m) => String(m.value) === String(val));
          if (m) displayValue = m.label;
        }
        rebuiltPairs.push({ key: key as keyof FilterCriteria, value: displayValue });
      }
    });
    setActivePairs(rebuiltPairs);
